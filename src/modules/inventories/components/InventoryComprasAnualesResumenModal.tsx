...
import { useInventoryComprasAnualesResumenModal } from "../hooks/useInventoryComprasAnualesResumenModal";
...
function InventoryComprasAnualesResumenModal() {
  const { isOpen, close, currentCode, rows, totals, fromDate, isLoading, error } = useInventoryComprasAnualesResumenModal();
  ...
  return (
    <ManagedWindowLayer windowId={MODAL_IDS.INVENTORY_COMPRAS_ANUALES_RESUMEN} isOpen={isOpen}>
      <section className="flex h-[min(560px,78vh)] w-[min(980px,92vw)] flex-col border border-[#2f8ce8] bg-[#ececec]">
        <header className="flex h-[30px] items-center justify-between border-b border-[#99a4af] bg-[#f0f0f0] px-2">
          <div className="flex items-center gap-1">
            ...
            <h2 className="text-[12px] leading-none font-semibold text-[#1e293b]">Compras anuales resumen</h2>
          </div>
        ...
        <div className="grid min-h-0 flex-1 grid-cols-[1fr]">
          <div className="relative modal-scroll min-h-0 overflow-auto border-r border-[#9ca3ab] border-b border-[#9ca3ab]">
            ...
            {isLoading ? <LegacyModalLoader label="Cargando compras anuales resumen..." /> : null}
          </div>
...
export default InventoryComprasAnualesResumenModal;
