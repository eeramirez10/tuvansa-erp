...
const emptySummaryRow = (): AnnualSummaryRow => ({
  year: 0,
  ene: 0,
  feb: 0,
  mar: 0,
...
const yearFromRow = (row: InventoryAnnualSaleRow): number => asNumber(row.year);

const rowTotal = (row: InventoryAnnualSaleRow): number =>
  asNumber(row.total) ||
  asNumber(row.ene) +
    asNumber(row.feb) +
    asNumber(row.mar) +
...
const aggregateByYear = (rows: InventoryAnnualSaleRow[]): AnnualSummaryRow[] => {
  const map = new Map<number, AnnualSummaryRow>();
...
const sumRows = (rows: AnnualSummaryRow[]): AnnualSummaryRow => {
...
export const useInventoryVentasAnualesResumenModal = () => {
  const { isOpen, close } = useModal(MODAL_IDS.INVENTORY_VENTAS_ANUALES_RESUMEN);
  ...
  const [rawRows, setRawRows] = useState<InventoryAnnualSaleRow[]>([]);
  ...
  useEffect(() => {
    ...
    const load = async () => {
      ...
      try {
        ...
        if (!isCancelled) {
          setRawRows([]);
          setError(loadError instanceof Error ? loadError.message : "Error cargando ventas anuales resumen.");
        }
      } finally {
        if (!isCancelled) {
          setIsLoading(false);
