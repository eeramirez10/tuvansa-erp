      ...
      <section className="flex h-[min(620px,84vh)] w-[min(1280px,98vw)] flex-col border border-[#2f8ce8] bg-[#ececec]">
        <header className="flex h-[30px] items-center justify-between border-b border-[#99a4af] bg-[#f0f0f0] px-2">
          <div className="flex items-center gap-1">
            ...
            <h2 className="text-[12px] leading-none font-semibold text-[#1e293b]">Ventas Anuales</h2>
          </div>
              ...
              {rows.map((row, index) => (
                <tr key={`${row.code}-${row.year}-${index}`} className="bg-[#efefef] odd:bg-[#f4f4f4]">
                  ...
                  <td className="w-[60px] border border-[#a6adb5] px-1 py-[4px] text-right">
                    {renderMonthValue(getRowTotal(row))}
                  </td>
                </tr>
              ))}
            </tbody>
        ...
        <footer className="flex h-[30px] items-center gap-[8px] border-t border-[#a1a8af] bg-[#ececec] px-2">
          <span className="text-[11px] text-[#223142]">Desde:</span>
          <span className="inline-flex h-[20px] w-[112px] items-center border border-[#a0a6ad] bg-[#d8d8d8] px-1 text-[11px] text-[#2f3943]">
            {fromDate}
          </span>
          <button
            type="button"
            className="inline-flex h-[24px] min-w-[88px] items-center justify-center border border-[#8f959c] bg-[#d8d8d8] px-2 text-[11px] text-[#1f2c3a]"
          >
            Imprimir
          </button>
        </footer>
...
export default InventoryVentasAnualesModal;
